...
func ParkingBookingActivity(ctx context.Context, req ParkingBookingRequest) (*ParkingBookingResult, error) {
	...
	case "booking-connection-error":
		// 一時的エラー（駐車場管理システム接続エラー）をシミュレート
		err := NewTemporalError("駐車場管理システムへの接続に失敗しました", "CONNECTION_ERROR")
	...
	default:
		...
		logger.Info("駐車場予約が完了", "BookingID", req.BookingID, "ResourceID", result.ResourceID)
		return result, nil
	}
}
